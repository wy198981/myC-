...
//		OneApmAgent.init(this.getApplicationContext()).setToken("5106067A94F46EB853A5042CF1F00C4E64").start();
        NBSAppAgent.setLicenseKey("a04ad42a66984f4391c6a23596a9dc9c")
                .withLocationServiceEnabled(true).start(this);// 听云适用于监控分析App的使用情况，和umeng统计功能类似
                    ...
                    case UPDATE:
                        ...
                        if (Integer.parseInt(versiontext) < Integer.parseInt(version_num))
                        {
                            ...
                            if (tempFile.exists())
                            {
                                ...
                                intentact.putExtras(bundle);// 传输了 tingchebaohd_hd.apk;
                            }
                            else
                            {
...
//		initImageLoader();  	// 初始化imageLoader；
    ...
    {
        versiontext = AppInfoUtil.getVersionCode(this);// 137
        tv_hello_version.setText(versiontext);

        Log.i(TAG, "versiontext:" + versiontext);
    }
    ...
    private Runnable runnable = new Runnable()
    {
        ...
        public void run()
        {
            ...
            try
            {
                ...
                String version = info.getVersion();// 124
                ...
                else
                {
                    ...
                    {
                        Log.i(TAG, "已是最新版,无需升级, 进入主界面"); // 127 > 124
                        loadMainUI();
                    }
                    else
                    {
            ...
            catch (Exception e)
            {
                e.printStackTrace();
                //是否是本地服务器
                if (!AppInfo.getInstance().getIsLocalServer(HelloActivity.this))
                {//是本地服务器则没有平板本地化的感念
                    //无网,网络超时  开启本地化
//					SharedPreferencesUtils.setParam(
