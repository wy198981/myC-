...
//			break;
    ...
    public void refreshView(CarNumberOrder order)
    {
        ...
        if ((
                (activity.isExitComeinCar && OrderListState.getInstance().isHandSearchState()) ||
...
//	}
    ...
    {
        //是否是本地服务器
        if (!AppInfo.getInstance().getIsLocalServer(activity))
        {//是本地服务器则没有平板本地化的感念
			/*本地化*/
...
//	}
    ...
    private void doPrePayOrderResult(String object)
    {
        ...
        if (prePayOrder.getResult().equals(Constant.sOne) || waitCollect == 0)
        {//成功
            String carnumber = itemOrder.getCarnumber();
        ...
        else if (prePayOrder.getResult().equals("2") || waitCollect > 0)
        {//需要补差价
            activity.cashFragment.hideCost();
            activity.cashFragment.showPrepay();
        ...
        else if (prePayOrder.getResult().equals("-1"))
        {//失败
