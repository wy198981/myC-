...
public class LoginActivity extends BaseActivity
{
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        IsNetWork.IsHaveInternet(LoginActivity.this); // TODO 测试是否有网络
        Intent intent = initStallUri(); // 获取url;
//		((application) getApplication()).setLoginActivity(this);
        initSqliteManager(); // TODO 获取数据库的实例
    ...
    private Intent initStallUri() // sd 卡下的安装包：tingchebaohd_hd.apk
    {
    ...
    private void tokenOperation(Intent intent)
    {
        ...
        {
            getAutoLoginInfo(); // 接收其他activity 来触发主动登录到服务器
        }
    }
    ...
    private void initSqliteManager()// TODO 数据库主要存储：停车场图片信息 账户信息 摄像头信息 ledinfo信息
    {
...
//					}
    ...
    public void initView()
    {
        ...
        // 加入usernames到ArrayList中,AutoCompleteTextView的下拉的列表信息
        ...
        // 本地服务器判断 和 ip信息
        FileUtil.writeSDFile("登录步骤", "accounts.size()=" + accounts.size()
                + "  是否是本地服务器" + SharedPreferencesUtils.getParam(LoginActivity.this, "nettype", "isLocalServer", false)
                + "  localip:" + SharedPreferencesUtils.getParam(LoginActivity.this, "nettype", "localip", null));
        // TODO 写到文件中供调试
    ...
    public void longinSuccess(
            final String username, final String MD5password, final String password,
    ...
    )
    {
        ...
        if (!ip.isEmpty())// TODO 带有本地的IP,是本地服务器?
        {// 输入ip不为null
...
//            Constant.requestUrl = "http://yxiudongyeahnet.vicp.cc:50803/zld/";
        ...
        String url = params.getRequstUrl(); // 根据username password version 工作站点来向服务器请求数据;
...
//            Log.e("---","handler 831 消息"+ (i++));
            ...
            if (msg.what == 831)
            {
                if (msg.obj.toString().equals("net"))
                {
                    HttpManager.requestLoginGET(login, loginurl, login, loginname, loginpass);// 等到有网络则主动请求
                }
                else
                {
                    Message m = new Message();
...
//		DialogManager.getInstance().showProgressDialog(this, "获取车场信息...");
    ...
    public boolean doSucess(String url, String object)
    {
        ...
        {
            doGetWorkInfoResult(object); // 获取工作站所有信息
        }
        return true;
    }
    ...
    public boolean doSucess(String url, String object, String username, String password, LoginInfo info)
    {
        // TODO Auto-generated method stub
        DialogManager.getInstance().dissMissProgressDialog();
        if (url.contains(Constant.COMINFO))// 获取车场所有的数据
        {
    ...
    @Override
    public boolean doSucess(String url, byte[] buffer, String username, String password)// 请求车辆登录信息
    {
        // TODO Auto-generated method stub
        dialog.dismiss();
    ...
    private void showPopupWindow(View parent)// 弹出框
    {
        int screenHeight;
        // TODO Auto-generated method stub
        if (listview == null)
        ...
        final ArrayList<String> selectAllAccount = sqliteManager.selectAllAccount();// 从数据库查询所有的用户信息
    ...
    private void intentInfo()// ZldNewActivity 需要大量的数据进行交互，直接从数据库中获取;
    {
